...
async function load() { try { await Promise.all([store.loadChapters(projectId), store.loadRelations(projectId)]) } catch { message.value = '先生成章节关系以开始编辑。' } }
async function generate() { busy.value = true; try { store.relations = await workflowApi.generateRelations(projectId); message.value = '章节关系已生成' } catch (error) { message.value = error instanceof Error ? error.message : '生成失败' } finally { busy.value = false } }
