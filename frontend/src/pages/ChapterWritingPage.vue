...
async function summary() {
  if (!selectedId.value) return
  busy.value = true
  try { await store.generateSummary(selectedId.value); message.value = '章节摘要已生成' }
  catch (error) { message.value = error instanceof Error ? error.message : '摘要生成失败' }
  finally { busy.value = false }
}
onMounted(async () => {
  await store.loadChapters(projectId)
  if (selectedId.value && store.chapters.some(chapter => chapter.id === selectedId.value)) await choose(selectedId.value)
  else if (store.chapters[0]) await choose(store.chapters[0].id)
})
